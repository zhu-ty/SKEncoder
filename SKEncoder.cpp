...
int SKEncoder::init(int frameNum, cv::Size imgSize, std::string fileName, FrameType type, int GpuID)
{
	...
	{
		SKCommon::warningOutput("Gpu id is not vaild, auto changed to the last Gpu");
		_GpuID = nGpu - 1;
	}
	ck(cuDeviceGet(&cuDevice, _GpuID));
	char szDeviceName[80];
	ck(cuDeviceGetName(szDeviceName, sizeof(szDeviceName), cuDevice));
	SKCommon::infoOutput(std::string("GPU in use: ") + szDeviceName);
...
int SKEncoder::endEncode()
{
	...
	{
		SKCommon::warningOutput("_encodedFrameNum not reached _frameNum");
	}
	_enc->DestroyEncoder();
	_fpOut.close();
